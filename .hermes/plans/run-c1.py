#!/usr/bin/env python3
"""Run Challenge 1 through MR-Krabs with audit + verify + retry enabled."""
...
result = orch.execute_with_judge(
    ...
    context={
        ...
        'spec': {
            'success_criteria': [
                'All 8+ pytest tests pass', 'Zero dependencies (stdlib only)',
...
with open(os.path.join(DIR, 'mrkrabs_result.json'), 'w') as f:
    json.dump({'challenge':'c1','success':result.get('success'),
