...
class ModelValidator:
    ...
    def validate_model(self, tier: str) -> tuple[bool, str, list[str]]:
        ...
        similar = [
            m for m in self._available_models if model_name in m.split("/")[-1] and m != model_id
        ][:3]
    ...
    def print_report(self, results: dict[str, dict]) -> None:
        ...
        print("=" * 60)
        print(
            f"Valid: {len(results['valid'])}, Invalid: {len(results['invalid'])}, Local: {len(results['local'])}"
        )
