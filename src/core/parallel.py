...
async def execute_task(task: ParallelTask) -> ParallelResult:
    ...
    try:
        ...
        return ParallelResult(name=task.name, success=True, result=result, duration=duration)
    except Exception as e:
        duration = time.time() - start
        return ParallelResult(name=task.name, success=False, error=str(e), duration=duration)
    ...
    for i, result in enumerate(results):
        if isinstance(result, Exception):
            processed_results.append(
                ParallelResult(
                    name=tasks[i].name, success=False, error=f"Unexpected error: {str(result)}"
                )
            )
        else:
...
def parallel_tool_executor(file_tools, tool_calls: list[dict[str, Any]]) -> dict[str, Any]:
    """
    ...
    for call in tool_calls:
        ...
        response = (
            f"{call['tool']}('{call.get('path', '')}', \"\"\"{call.get('content', '')}\"\"\")"
        )
