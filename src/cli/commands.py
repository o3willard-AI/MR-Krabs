...
from src.core.cost import CostTracker, Budget, TokenCount, BudgetExceededError, FailureMode
from src.core.orchestrator import LLMOrchestrator, MODELS
from src.core.config import load_config, config_to_budget, DEFAULT_CONFIG
from src.validators.startup import StartupValidator


def cmd_init(config_path: Optional[Path] = None) -> int:
...
def cmd_doctor() -> int:
    ...
    api_key = os.environ.get("OPENROUTER_API_KEY")
    if api_key:
        print(f"[PASS] OPENROUTER_API_KEY is set")
    else:
        print(f"[FAIL] OPENROUTER_API_KEY is not set")
        print(f"       Fix: export OPENROUTER_API_KEY='your-key'")
        all_ok = False
    ...
    print(f"[PASS] Budget: ${float(budget.daily_limit_usd):.2f}/day (mode: {budget.failure_mode.value})")
...
def cmd_dry_run(description: str, tier: Optional[str] = None) -> int:
    ...
    print(f"  Estimated cost: ${float(min_cost):.4f} (L0) to ${float(min_cost) * 10:.4f} (if escalated)")
...
def cmd_stats() -> int:
    """Show cost summary."""
    from src.core.config import load_config, config_to_budget
    ...
    print(f"Today's spending: ${summary['daily_total']:.2f} / ${summary['budget_limit']:.2f} ({summary['budget_used_percent']:.1f}%)")
    print()
...
def cmd_explain(task_id: str, log_dir: Optional[Path] = None) -> int:
    ...
    if not handoffs_dir.exists() and not escalations_dir.exists():
        print(f"No execution logs found. Run tasks first to generate logs.")
        return 1

    safe_task_id = task_id.replace(".", "_")
    handoff_files = sorted(handoffs_dir.glob(f"{safe_task_id}-*.json")) if handoffs_dir.exists() else []
    escalation_files = sorted(escalations_dir.glob(f"{safe_task_id}-*.json")) if escalations_dir.exists() else []
    ...
    for f in escalation_files:
        ...
        print(f"    Action: Escalated to next tier")
        print()
